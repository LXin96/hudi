...
public class Pipelines {
  ...
  public static DataStreamSink<Object> bulkInsert(Configuration conf, RowType rowType, DataStream<RowData> dataStream) {
    ...
    // TODO 如果hudie的索引类型是桶索引类型   桶索引显然是分区内唯一
    if (OptionsResolver.isBucketIndexType(conf)) {
      String indexKeys = conf.getString(FlinkOptions.INDEX_KEY_FIELD); //TODO 获取索引字段 默认为空字符串，支持nested 字段
      int numBuckets = conf.getInteger(FlinkOptions.BUCKET_INDEX_NUM_BUCKETS); //TODO 获取hudi每个分区的桶的数量 默认为4

      BucketIndexPartitioner<HoodieKey> partitioner = new BucketIndexPartitioner<>(numBuckets, indexKeys); //记录分区 持有那个分区那个桶
      RowDataKeyGen keyGen = RowDataKeyGen.instance(conf, rowType); // TODO keyGen ？？ 这个key指的就是recordKey
      RowType rowTypeWithFileId = BucketBulkInsertWriterHelper.rowTypeWithFileId(rowType); //TODO 这个Row信息包含了 组成[文件组元字段+record的schema类型]
      InternalTypeInfo<RowData> typeInfo = InternalTypeInfo.of(rowTypeWithFileId);//TODO 推导出RowData的数据信息 转换后的类型对象

      Map<String, String> bucketIdToFileId = new HashMap<>(); // TODO 记录了 bucketIdToFileId 的映射
      ...
      if (conf.getBoolean(FlinkOptions.WRITE_BULK_INSERT_SORT_INPUT)) { // TODO 是否根据某些特定的字段进行排序
        ...
        dataStream = dataStream.transform("file_sorter", typeInfo, sortOperatorGen.createSortOperator())
            .setParallelism(conf.getInteger(FlinkOptions.WRITE_TASKS)); // same parallelism as write task to avoid shuffle TODO 和写任务并行度一致 避免发生shuffle
